...
- **No self-calls, no sandboxed CLI.** An agent must not send work to its own lane (Codex → `codex`), since that starts a second session on the same quota with none of the conversation. The CLI detects its caller (`CODEX_THREAD_ID`, `CLAUDECODE`, or the process tree; `AGENTCTL_CALLER` overrides). It keeps the caller's lanes out of routing and refuses `--to <own lane>` unless you pass `--allow-self`. From a sandbox with network disabled (`CODEX_SANDBOX_NETWORK_DISABLED=1`) the CLI fails at once instead of hanging, so use the MCP tools, which run outside the sandbox. An agentctl MCP server started by an agentctl worker detects this from the process tree and refuses to dispatch, so nesting can't recurse. `agentctl doctor` shows all of this.
