...
export function resolveSessiongraphRoot(): string | null {
  const root = setting('SESSIONGRAPH_ROOT')?.trim()
...
export async function runSessiongraphCli(args: string[], root?: string): Promise<SessiongraphRunResult> {
  ...
  if (existsSync(join(pkg, 'src', 'sessiongraph', 'cli.py'))) {
    const result = await execa(setting('SESSIONGRAPH_PYTHON')?.trim() || 'python3', [
  ...
  const uvBin = setting('SESSIONGRAPH_UV')?.trim() || 'uv';
  const frozen = setting('SESSIONGRAPH_UV_FROZEN') !== '0';
  const uvArgs = frozen
