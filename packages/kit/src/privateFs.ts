 ...
 * directories it creates. Also tightens the app's home when `path` is in it.
 */
export function ensurePrivateDir(path: string, home?: string): void {
  mkdirSync(path, { recursive: true, mode: 0o700 });
  tighten(path, 0o700);
  if (home && isInside(path, home)) tighten(home, 0o700);
}
